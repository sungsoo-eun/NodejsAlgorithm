  ...
  (function() {
    $oj$$25$$.$__registerWidget$("oj.ojTrain", $$$$23$$.oj.baseComponent, {version:"1.0.0", defaultElement:"\x3cdiv\x3e", widgetEventPrefix:"oj", options:{steps:[], selected:"", optionChange:null, beforeDeselect:null, deselect:null, beforeSelect:null, select:null}, $_stepNum$:0, $_stepArray$:null, _ComponentCreate:function() {
      ...
      if (this.$_stepArray$[$index$$180$$]) {
        ...
        this.$_stepArray$[$index$$180$$][2] || this.$_selectedIndex$ === $index$$180$$ || (this._hoverable($button$$1$$), $button$$1$$.on("click" + this.eventNamespace, function($event$$370$$) {
          $self$$143$$.$_fireOptionChange$("selected", $self$$143$$.options.selected, this.parentNode.parentNode.id, $event$$370$$);
          $self$$143$$.refresh();
        }));
      ...
      if (this.$_stepArray$[$index$$181$$]) {
        ...
        this.$_stepArray$[$index$$181$$][2] || this.$_selectedIndex$ === $index$$181$$ || (this._hoverable($icon$$6$$), $icon$$6$$.on("click" + this.eventNamespace, function($event$$371$$) {
          $self$$144$$.$_fireOptionChange$("selected", $self$$144$$.options.selected, this.parentNode.parentNode.parentNode.id, $event$$371$$);
          $self$$144$$.refresh();
        }));
    ...
    }, $_fireOptionChange$:function($eventData$$8_key$$132$$, $previousValue$$1_stepIndex$$, $value$$252$$, $originalEvent$$1$$) {
      $eventData$$8_key$$132$$ = {option:$eventData$$8_key$$132$$, fromStep:this.getStep($previousValue$$1_stepIndex$$), toStep:this.getStep($value$$252$$), optionMetadata:{writeback:$originalEvent$$1$$ ? "shouldWrite" : "shouldNotWrite"}};
      !1 !== this._trigger("beforeDeselect", $originalEvent$$1$$, $eventData$$8_key$$132$$) && !1 !== this._trigger("beforeSelect", $originalEvent$$1$$, $eventData$$8_key$$132$$) && ($previousValue$$1_stepIndex$$ = this.$_getStepIndex$($previousValue$$1_stepIndex$$), -1 !== $previousValue$$1_stepIndex$$ && (this.options.steps[$previousValue$$1_stepIndex$$].visited = !0), this._trigger("deselect", $originalEvent$$1$$, $eventData$$8_key$$132$$), this.option("selected", $value$$252$$, {_context:{originalEvent:$originalEvent$$1$$,
      ...
      if (this.$_stepArray$[$index$$183$$]) {
        ...
        $disabled$$9$$ || ($label$$13_stepLi$$1$$.attr("href", "#"), this._hoverable($label$$13_stepLi$$1$$), $label$$13_stepLi$$1$$.on("click keydown" + this.eventNamespace, function($event$$372$$) {
          if ($event$$372$$.keyCode === $$$$23$$.ui.keyCode.ENTER || "click" === $event$$372$$.type) {
            $event$$372$$.preventDefault(), $self$$145$$.$_fireOptionChange$("selected", $self$$145$$.options.selected, this.parentNode.parentNode.id, $event$$372$$), $self$$145$$.refresh(), $event$$372$$.keyCode === $$$$23$$.ui.keyCode.ENTER && $self$$145$$.$_setFocus$(this.parentNode.parentNode.id);
          }
        }));
      ...
      this.refresh();
    }, _setOption:function($key$$133$$, $value$$253$$, $flags$$35$$) {
      "selected" === $key$$133$$ && this.options.selected && this.$_stepArray$ && this.$_stepArray$[this.$_selectedIndex$] && this.$_fireOptionChange$("selected", this.$_stepArray$[this.$_selectedIndex$][1], $value$$253$$, null);
      this._super($key$$133$$, $value$$253$$, $flags$$35$$);
    }, refresh:function() {
      this._super();
      this._destroy();
      this.$_setupTrain$();
      ...
      for (var $stepNum$$ = this.$_stepArray$ ? this.$_stepArray$.length : 0, $stepIndex$$3$$ = 0;$stepIndex$$3$$ < $stepNum$$;$stepIndex$$3$$++) {
        ...
        if ($node$$84$$ === this.getNodeBySubId($stepLocator$$)) {
          return $stepLocator$$;
        }
      }
      return null;
